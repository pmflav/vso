        ...
        <div className="p-8 space-y-8 max-w-6xl mx-auto">
            <header className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-slate-400 mt-1">AI-powered funding opportunities for your business.</p>
                </div>
                <button
                    onClick={handleScout}
                    disabled={scouting}
                    className="bg-purple-600 hover:bg-purple-500 text-white px-6 py-3 rounded-lg text-sm font-semibold transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-purple-900/20"
                >
                    {scouting ? (
                        <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl">
                    ...
                    <p className="text-3xl font-bold text-white mt-2">{grants.length}</p>
                </div>
                <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl">
                    <h4 className="text-slate-500 text-sm font-medium uppercase">Potential Funding</h4>
                    <p className="text-3xl font-bold text-emerald-400 mt-2">~$1.5M</p>
                </div>
                <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl">
            ...
            <div className="space-y-4">
                ...
                {loading ? (
                    <div className="py-12 text-center text-slate-500">Loading grants...</div>
                ) : grants.length === 0 ? (
                    <div className="bg-slate-900/50 border border-dashed border-slate-800 rounded-xl p-12 text-center space-y-4">
                        <div className="text-4xl">📭</div>
                        <h3 className="text-white font-medium">No grants found yet</h3>
...
function GrantCard({ grant }: { grant: any }) {
    return (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 hover:border-purple-500/30 transition-all group relative overflow-hidden">
            ...
            <div className="flex items-start justify-between relative z-10">
                <div className="space-y-2">
                    <div className="flex items-center gap-3">
                        <h3 className="text-xl font-bold text-white group-hover:text-purple-400 transition-colors">{grant.title}</h3>
                        <span className={`px-2 py-0.5 rounded text-xs font-bold ${grant.match_score >= 90 ? 'bg-emerald-500/10 text-emerald-400' :
                            grant.match_score >= 70 ? 'bg-amber-500/10 text-amber-400' : 'bg-slate-800 text-slate-400'
                            }`}>
                            {grant.match_score}% Match
                        </span>
                    </div>
                    <p className="text-slate-400 max-w-2xl text-sm leading-relaxed">{grant.description}</p>
                    ...
                    <div className="flex items-center gap-6 pt-2 text-sm">
                        ...
                        <div className="text-slate-300">
                            <span className="text-slate-500 mr-2">Deadline:</span>
